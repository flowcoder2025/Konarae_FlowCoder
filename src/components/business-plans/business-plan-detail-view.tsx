...
interface BusinessPlanDetailViewProps {
  initialData: BusinessPlanDetail;
}
...
export function BusinessPlanDetailView({ initialData }: BusinessPlanDetailViewProps) {
  const router = useRouter();
  const id = initialData.id;

  const [businessPlan, setBusinessPlan] = useState<BusinessPlanDetail>(initialData);
  ...
  const handleGenerateAll = async () => {
    ...
    try {
      ...
      toast.success("사업계획서가 성공적으로 생성되었습니다.");
    } catch (error) {
      logger.error("Generate business plan error", { error });
      toast.error("사업계획서 생성에 실패했습니다. 잠시 후 다시 시도해주세요.");
    } finally {
  ...
  return (
    <div className="container mx-auto py-8 max-w-6xl">
      ...
      <div className="flex gap-3 mb-6">
        ...
        <Button onClick={() => setShowHwpModal(true)} variant="outline">
          <FileDown className="h-4 w-4 mr-2" />
          HWP 내보내기
        </Button>
      </div>
      ...
      <Dialog open={showHwpModal} onOpenChange={setShowHwpModal}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button onClick={() => setShowHwpModal(false)}>확인</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <CardContent className="py-8">
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">
                ...
                <span>페이지를 떠나도 서버에서 생성이 계속됩니다</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {businessPlan.sections && businessPlan.sections.length > 0 ? (
        <div className="space-y-6">
          ...
          <div className="flex justify-center pt-4">
            <Button variant="outline" onClick={() => setShowAddSectionModal(true)}>
      ...
      <Dialog open={showAddSectionModal} onOpenChange={setShowAddSectionModal}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            ...
            <DialogDescription>추가할 섹션의 제목을 입력해주세요.</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <div className="space-y-2">
