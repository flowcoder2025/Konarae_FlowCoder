...
export function DiagnosisDetailView({ initialData }: DiagnosisDetailViewProps) {
  const router = useRouter();
  const [diagnosis, setDiagnosis] = useState<DiagnosisResponse>(initialData);

  // processing 상태일 때만 폴링
  useEffect(() => {
    if (diagnosis.status !== "processing") return;

    const interval = setInterval(async () => {
      try {
        const res = await fetch(`/api/diagnosis/${diagnosis.id}`);
        ...
        <div className="mt-6 space-y-6">
          ...
          <Card>
            <CardContent className="py-6">
              ...
              <Progress value={diagnosis.fitScore} className="mt-4 h-3" />
            </CardContent>
          </Card>
