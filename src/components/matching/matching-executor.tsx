...
import { Loader2, Target, CheckCircle, AlertCircle } from "lucide-react";
...
interface MatchingResult {
  ...
  totalScore: number;
  businessSimilarityScore: number;
...
export function MatchingExecutor({ companyId, companyName }: MatchingExecutorProps) {
      ...
      {!results && (
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>
              AI가 {companyName}의 정보와 지원사업을 분석하여 최적의 매칭 결과를 제공합니다
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button size="lg" onClick={executeMatching} disabled={loading}>
              {loading ? (
                <>
